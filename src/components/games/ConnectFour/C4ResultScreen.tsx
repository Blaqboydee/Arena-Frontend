...
import type { C4MatchOver } from "../../../hooks/useConnectFour";
...
type Props = {
  matchOver: C4MatchOver;
  players:   Player[];
  myId:      string;
};

export default function C4ResultScreen({ matchOver, players, myId }: Props) {
  ...
  const shareText = isDraw
    ? `Drew ${myScore}-${oppScore} after ${matchOver.totalRounds} rounds in Connect Four on ARENA ⚡ arenagameplay.vercel.app`
    : iWon
    ? `Beat ${opponent?.name ?? "my opponent"} ${myScore}-${oppScore} in Connect Four on ARENA ⚡ arenagameplay.vercel.app`
    : `Lost to ${matchOver.winnerName} ${oppScore}-${myScore} in Connect Four on ARENA ⚡ arenagameplay.vercel.app`;
  ...
  const tagline = isDraw
    ...
    : iWon
    ? "Four in a row!"
    : `${matchOver.winnerName} connected four.`;
        ...
        <div className="bracket-card w-full bg-surface border border-border rounded-sm p-6 flex flex-col gap-5">
          ...
          <p className="text-center font-mono text-xs text-muted">
            {isDraw
              ...
              : iWon
              ? "Four in a row! Challenge someone new."
              : `${matchOver.winnerName} connected four. Rematch?`}
          </p>
        </div>
