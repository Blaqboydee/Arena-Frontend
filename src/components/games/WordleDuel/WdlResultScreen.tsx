...
import type { WdlMatchOver } from "../../../hooks/useWordleDuel";
...
type Props = {
  matchOver: WdlMatchOver;
  players:   Player[];
  myId:      string;
};

export default function WdlResultScreen({ matchOver, players, myId }: Props) {
  ...
  const shareText = isDraw
    ? `Drew ${myScore}-${oppScore} after ${matchOver.totalRounds} rounds in Wordle Duel on ARENA ⚡ arenagameplay.vercel.app`
    : iWon
    ? `Beat ${opponent?.name ?? "my opponent"} ${myScore}-${oppScore} in Wordle Duel on ARENA ⚡ arenagameplay.vercel.app`
    : `Lost to ${matchOver.winnerName} ${oppScore}-${myScore} in Wordle Duel on ARENA ⚡ arenagameplay.vercel.app`;
  ...
  const tagline = isDraw
    ...
    : iWon
    ? "Word wizard!"
    : `${matchOver.winnerName} cracked more words.`;
        ...
        <div className="bracket-card w-full bg-surface border border-border rounded-sm p-6 flex flex-col gap-5">
          ...
          <p className="text-center font-mono text-xs text-muted">
            {isDraw
              ...
              : iWon
              ? "Word wizard! Challenge someone new."
              : `${matchOver.winnerName} cracked more words. Rematch?`}
          </p>
        </div>
